...
const TYPE_LABEL = { saving: '储蓄账户', ew: 'eWallet', cc: '信用卡' };
...
function identifierFor(account) {
  if (account.type === 'cc') return account.creditCardLast4 ? `•••• ${account.creditCardLast4}` : '';
  if (account.type === 'ew') return maskAssetIdentifier(account.walletIdentifier);
  return maskAssetIdentifier(account.bankAccountNumber || account.debitCardNumber);
}
...
function accountEditorHTML(account, type) {
  const credit = type === 'cc';
  const wallet = type === 'ew';
  const identifierFields = credit
    ? `<label class="asset-form-field"><span>信用卡末四位（可选）</span><input name="creditCardLast4" inputmode="numeric" maxlength="4" value="${escapeHTML(account?.creditCardLast4 || '')}" placeholder="例如 9910" /></label>`
    : wallet
      ? `<label class="asset-form-field"><span>钱包手机 / 账户标识（可选）</span><input name="walletIdentifier" maxlength="60" value="${escapeHTML(account?.walletIdentifier || '')}" placeholder="例如 手机号码或钱包用户名" /></label>`
      : `<label class="asset-form-field"><span>银行账号（可选）</span><input name="bankAccountNumber" inputmode="numeric" maxlength="80" value="${escapeHTML(account?.bankAccountNumber || '')}" placeholder="银行账号" autocomplete="off" /></label><label class="asset-form-field"><span>Debit Card 号码（可选）</span><input name="debitCardNumber" inputmode="numeric" maxlength="80" value="${escapeHTML(account?.debitCardNumber || '')}" placeholder="Debit Card 号码" autocomplete="off" /></label>`;
  const financial = credit
    ? `<label class="asset-form-field"><span>信用额度</span><div class="asset-money-input"><b>RM</b><input name="limit" inputmode="decimal" value="${account?.limit ?? ''}" placeholder="0.00" required /></div></label>
       <label class="asset-form-field"><span>卡组织</span><select name="network"><option value="">未指定</option>${['Visa', 'Mastercard', 'American Express', 'Other'].map((value) => `<option value="${value}" ${account?.network === value ? 'selected' : ''}>${value === 'Other' ? '其他' : value}</option>`).join('')}</select></label>
       <label class="asset-form-field"><span>等级</span><select name="tier"><option value="">未指定</option>${['Classic', 'Gold', 'Platinum', 'Signature', 'Infinite', 'World', 'Other'].map((value) => `<option value="${value}" ${account?.tier === value ? 'selected' : ''}>${value === 'Other' ? '其他' : value}</option>`).join('')}</select></label>
  ...
  return `<form class="asset-editor-form" data-asset-editor>
    <label class="asset-form-field"><span>名称</span><input name="name" maxlength="40" value="${escapeHTML(account?.name || '')}" placeholder="例如 日常账户" required /></label>
    <label class="asset-form-field"><span>${credit ? '发卡机构' : wallet ? '钱包品牌' : '银行'}</span><input name="bank" maxlength="50" value="${escapeHTML(account?.bank || '')}" placeholder="可稍后配对官方资料" /></label>
...
export function openAssetEditor({ accountId = null, type = 'saving', stacked = false, returnToManager = false } = {}) {
  ...
  openSheet({
    ...
    contentHTML: accountEditorHTML(account, editorContext.type),
    onOpen(sheet) {
...
function managerRows(type) {
  ...
  return list.map((account) => {
    ...
    return `<div class="asset-manage-row${active ? '' : ' is-archived'}" data-id="${escapeHTML(account.id)}" data-active="${active}">
      ...
      <button type="button" class="asset-manage-main" data-action="asset-manager-edit" data-id="${escapeHTML(account.id)}"><span class="asset-manage-icon">${icon(account.type === 'cc' ? 'wallet' : 'assets', 17)}</span><span><strong>${escapeHTML(account.name)}</strong><small>${escapeHTML(detail)}</small></span>${icon('chevronRight', 15)}</button>
...
export function registerAssetManagementActions() {
  ...
  registerAction('asset-editor-save', (el) => {
    ...
    try {
      const changes = { name: values.name, bank: values.bank, note: values.note, isDefault: form.elements.isDefault.checked, isHidden: form.elements.isHidden.checked };
      if (editorContext.type === 'saving') Object.assign(changes, { bankAccountNumber: values.bankAccountNumber, debitCardNumber: values.debitCardNumber });
      if (editorContext.type === 'ew') changes.walletIdentifier = values.walletIdentifier;
      if (editorContext.type === 'cc') Object.assign(changes, { creditCardLast4: values.creditCardLast4, limit: values.limit, sharedLimitPoolId: values.sharedLimitPoolId || null, network: values.network, tier: values.tier, includeInTotalDebt: form.elements.includeInTotalDebt.checked });
